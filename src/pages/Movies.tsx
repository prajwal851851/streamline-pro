...
const Movies = () => {
  ...
  const { data, isLoading, isError } = useQuery<StreamingMovie[]>({
    queryKey: ["streaming-movies", "movie"],
    queryFn: () => fetchStreamingMoviesByType("movie"),
  });
  ...
  return (
    <div className="min-h-screen pt-20 px-6 pb-12">
      <div className="max-w-7xl mx-auto animate-fade-in space-y-6">
        ...
        {isError && <p className="text-destructive">Could not load movies.</p>}

        {!isLoading && !isError && movies.length === 0 && (
          <p className="text-muted-foreground">No movies available yet. Run the scraper to add some.</p>
        )}
