  ...
  return (
    <div className="min-h-screen pt-20 px-6 pb-12">
      <div className="max-w-7xl mx-auto animate-fade-in space-y-6">
        ...
        {isLoading && <p className="text-muted-foreground">Loading streaming catalog...</p>}
        {isError && <p className="text-destructive">Could not load streaming catalog.</p>}

        {!isLoading && !isError && movies.length === 0 && (
          <p className="text-muted-foreground">No streaming items yet. Run the scraper to add some.</p>
        )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
          {movies.map((movie) => (
            <button
              key={movie.id}
              onClick={() => navigate(`/streaming/${movie.id}`)}
